...
export default function Navbar(props) {
  ...
  return(
    <nav id="container-navbar" className="fixed top-0 left-0 right-0 z-50">
      <div id="navbar" className="relative px-10 py-2 md:py-5 flex flex-row flex-nowrap justify-between items-center ease-in-out duration-300">
        <ul className={`max-lg:hidden relative lg:w-[calc(100%-16px)] xl:w-[calc(100%-48px)] 2xl:w-[calc(100%-60px)] lg:px-2 xl:px-6 2xl:px-[30px] flex flex-row flex-nowrap gap-x-4 lg:gap-x-5 2xl:gap-x-6 font-resolute text-[17px] tracking-[1px] leading-[26px] font-normal box-border ${listColorNav[props?.page].text}`}>
        ...
        <div className="max-lg:hidden relative w-full flex flex-row flex-nowrap justify-end">
          <Button
            ...
            typeColor="primary"
            text="ÚNETE AHORA"
            customClass="text-base rounded-xl"
          />
        </div>
